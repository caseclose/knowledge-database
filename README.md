...
graph LR
    ...
    class GPU,OC,GUI,CC,MCP,RA,LC,PI,AD,PE1,EV1,DP1,DP2,DP3,OPD,RLPT,IE,PT,DT,Diff,MM,PEnc,MoE,Norm,Attn,WeLM sub
    class K1,K2,K3,K4,K5,K6,K7,K8,K9,K10,K11,K12,K13,K14,K15,K16,K17,K18,K19,K20,K21,K22,K23,K24,K25,K26,K27,K28,K29,K30 knowledge
    ...
    click K30 href "#/model-architecture/welm/welm-parameter-scales"
```
...
2. **元信息**：标题下一行用引用写出创建/更新时间，可选标签：
   ...
   多个标签用顿号分隔，如 `标签：面试、后训练`。预训练 / 后训练用这两个词，不要用笼统的「训练」。
...
| `model-training/rl-post-training/grpo-advantage-function.md` | GRPO：同题多答几遍，比组内平均好的加强、差的压低；clip + KL 防止改太猛 |
| `model-training/rl-post-training/dapo-vs-grpo.md` | DAPO：GRPO 四补丁（更高上沿 / 只练有对有错 / token 平均 / 超长整形）并去掉 KL |
...
| `model-architecture/diffusion/classifier-free-guidance.md` | CFG：训练随机丢条件，推理 $\varepsilon_\varnothing+s(\varepsilon_c-\varepsilon_\varnothing)$ 放大听 prompt 的方向 |
