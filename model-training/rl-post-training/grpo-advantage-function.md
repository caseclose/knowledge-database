...
### 超过 clip 会把这条 rollout 丢掉吗？

不会。这里容易混：clip 卡的不是「回答有多好」，也不是 reward / advantage 有多大，而是**新旧策略生成同一个 token 的概率比** $r_{i,t}$。

一份好回答 $\hat{A}_i>0$，$\varepsilon=0.2$。若训练把某个 token 的概率比推到 $r_{i,t}=1.35$：

$$
\min(1.35\,\hat{A}_i,\; \operatorname{clip}(1.35, 0.8, 1.2)\,\hat{A}_i)
=
\min(1.35\,\hat{A}_i,\; 1.2\,\hat{A}_i)
=
1.2\,\hat{A}_i
$$
...
- 这条 rollout **仍留在 batch 里**，reward 和 advantage 照常算；
- 超过上沿后，**这个 token** 的策略梯度不再继续奖励「把概率再抬高」；
- 同一回答里其他还没越界的 token，照样可以出梯度；
- KL、熵等其他项也仍可能出梯度。

更准确的说法：

> 不是「好回答被丢弃」，而是「这个好回答已经把相关 token 的概率推得够多，所以暂时停止进一步强化」。

reward 极高、$\hat{A}_i$ 很大，**不会**单纯因为优势大就被 clip。较大的 $\hat{A}_i$ 只是可能让模型更新得更快，从而在后续更新里更容易把 $r_{i,t}$ 推过界。

反方向也一样：

- $\hat{A}_i>0$ 且 $r_{i,t}>1+\varepsilon$：停止继续提高概率；
- $\hat{A}_i<0$ 且 $r_{i,t}<1-\varepsilon$：停止继续降低概率。

标准 GRPO 是**逐 token clip**，不是把整份回答扔掉。只有某些 sequence-level 重要性采样变体才会按整条序列判断。刚完成 rollout 时一般 $r_{i,t}\approx 1$，clip 主要在**同一批 rollout 被重复优化多轮**之后才真正咬住。[DeepSeekMath](https://arxiv.org/abs/2402.03300)、[TRL GRPO](https://huggingface.co/docs/trl/grpo_trainer)
